...
**อัปเดต:** ตัวเลข 300 calls/day ด้านล่างนี้คือโควตาเก่าของ SolarEdge v1 (`?api_key=`) ที่เลิกใช้ไปแล้วตั้งแต่ย้ายมา v2 (ดูหัวคอมเมนต์ของ `src/services/solarEdgeService.ts`) ตอนนี้เบราว์เซอร์ไม่ได้คุยกับ SolarEdge โดยตรงอีกต่อไป — `DAILY_QUOTA_LIMIT` (= 5000/วัน) นับเฉพาะจำนวนครั้งที่เบราว์เซอร์ poll เข้า `/api/solaredge` ของ backend เราเอง ไม่ใช่จำนวนครั้งที่ backend เรียก SolarEdge จริง เพดานที่คุมฝั่งต้นทางจริง ๆ คือ `SOLAREDGE_MONTHLY_CALL_BUDGET` ของ backend (100,000 ครั้ง/เดือน, `worker/wrangler.toml:19-23`) ซึ่งที่ cadence default 3600 วินาที (1 ชั่วโมง) บอร์ดนี้ (7 registration บนหมุด 5 วิทยาเขต) ใช้เพียง ~847 ครั้ง/วัน = **~25,000 ครั้ง/เดือน** — เหลือที่สบาย เพราะเลย 900 วินาทีไปแล้ว ชุดกราฟ/พลังงาน (ที่ตรึงไว้ราย 15 นาที) จะถูกดึงตามจังหวะ tick ของบอร์ดไปด้วย คือชั่วโมงละครั้ง ไม่ใช่ 15 นาทีครั้ง เทียบกับ default ก่อนหน้า 900 วินาที ที่ใช้ ~101,000 และของเดิม 300 วินาที ที่ใช้ ~141,000 เหตุผลของ bulk endpoint ด้านล่างจึงยังสำคัญเหมือนเดิม เพียงแต่เพดานที่ต้องระวังคืออันนี้ ไม่ใช่ 300/วันแบบเดิม และถ้าหรี่ interval ลงมา ตัวเลขนี้จะพุ่งทันที (ที่ floor 10 วินาที = ~63,000/วัน หมดงบในวันครึ่ง) — แผงตั้งค่าคำนวณให้ดูสด ๆ ทั้ง ต่อนาที/ต่อวัน/ต่อเดือน

| วิธี | เรียก SolarEdge ต่อรอบ poll | ผลลัพธ์ เทียบกับ `SOLAREDGE_MONTHLY_CALL_BUDGET` (100,000/เดือน) |
|---|---|---|
| ยิงทีละไซต์ (เดิม) | 1 ครั้ง/ไซต์ (5 ไซต์ = 5 ครั้ง/รอบ) | ยิ่งเพิ่มไซต์ยิ่งเปลืองโควตาเดือนเป็นเส้นตรง — เกินเพดานเร็วกว่าหลายเท่า |
| Bulk endpoint (ปัจจุบัน) | รวมหลายไซต์ในคำขอเดียว | ที่ cadence default (3600 วิ) ใช้ **~25,000/เดือน** — เหลือที่สบาย (เทียบ 900 วิ = ~101,000 และ 300 วิ = ~141,000) |
...
และมี `QUOTA_RESERVE` กันโควตาหมดกลางงาน (ปัจจุบันคือ 5% ของ `DAILY_QUOTA_LIMIT` ขั้นต่ำ 20 ครั้ง — ดู `src/services/solarEdgeService.ts`; จะเสิร์ฟข้อมูลจากแคชแทน)
...
|---|---|---|
| ช่วง poll SolarEdge (กำลังผลิต Real-time) | ตั้งในหน้า Settings; ค่า default อยู่ที่ `src/types.ts` → `DEFAULT_REFRESH_INTERVALS.powerFlowSec` (และ backend `worker/src/config.ts` → `DEFAULT_POWER_FLOW_INTERVAL_SEC`) | 1 ชม. (3600 วิ) — เลย 900 วิ แล้วค่านี้คุมจังหวะทั้งบอร์ด ไม่ใช่แค่ตัวเลข kW |
